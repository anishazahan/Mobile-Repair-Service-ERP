...
const TECH_PHOTOS: Record<string, string> = {
  "TECH-01":
    "https://images.unsplash.com/photo-1560249956-b3731ecf3153?q=85&w=1200&auto=format&fit=crop",
  "TECH-02":
    "https://images.unsplash.com/photo-1699389795119-297f8af9111f?q=85&w=1200&auto=format&fit=crop",
};

export function TeamPreviewSection() {
  const featured = db.technicians.filter((t) =>
    ["TECH-01", "TECH-02"].includes(t.id),
  );
  ...
  return (
    <section className="container py-20">
      <div className="grid items-center gap-16 lg:grid-cols-[1.1fr_1fr]">
        <Reveal
          direction="left"
          className="relative mx-auto h-[420px] w-full max-w-lg sm:h-[500px] lg:h-[600px] lg:max-w-none"
        >
            ...
            <div className="absolute inset-x-0 top-0 bg-gradient-to-b from-black/70 to-transparent p-4">
              <figcaption>
                ...
                <p className="text-xs text-white/80">
                  {humanizeStatus(featured[0]?.specialties[0] ?? "")} Specialist
                </p>
              </figcaption>
            </div>
          </figure>
          ...
          <figure className="group absolute bottom-0 right-0 z-10 h-[68%] w-[68%] overflow-hidden border-4 border-primary shadow-xl">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-4">
              <figcaption>
                ...
                <p className="text-xs text-white/80">
                  {humanizeStatus(featured[1]?.specialties[0] ?? "")} Specialist
                </p>
              </figcaption>
            </div>
          </figure>
        ...
        <Reveal direction="right" delay={100} className="space-y-5">
          ...
          <p className="text-muted-foreground">
            Every technician on the floor is certified across the major device
            brands, and every job is double-checked before it goes back out the
            door — that's who's actually working on your device.
          </p>
          <Button
