...
export function useStaffRows(filters?: StaffFilters) {
  return useQuery({ queryKey: [...STAFF_KEY, "rows", filters], queryFn: () => api.getStaffRows(filters) });
}

export function useStaffMember(id: string) {
  return useQuery({ queryKey: staffKey(id), queryFn: () => api.getStaffMember(id), enabled: Boolean(id) });
}

function invalidateStaff(queryClient: ReturnType<typeof useQueryClient>) {
  queryClient.invalidateQueries({ queryKey: STAFF_KEY });
  // Linking/unlinking a technician profile shows up on that technician's
  // own detail page too.
...
export function useCreateStaffUser() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not add the staff account."),
  });
}
...
export function useUpdateStaffUser(id: string) {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not update the staff account."),
  });
}
...
export function useSetStaffStatus(id: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (status: "active" | "inactive" | "suspended") => api.setStaffStatus(id, status),
    onSuccess: (user) => {
      invalidateStaff(queryClient);
