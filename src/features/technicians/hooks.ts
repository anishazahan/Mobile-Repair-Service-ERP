...
export function useTechnicianRows(filters?: TechnicianFilters) {
  return useQuery({
    queryKey: [...TECHNICIANS_KEY, "rows", filters],
    queryFn: () => api.getTechnicianRows(filters),
  });
}

export function useTechnician(id: string) {
  return useQuery({
    queryKey: technicianKey(id),
...
export function useCreateTechnician() {
  ...
  return useMutation({
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error ? err.message : "Could not add the technician.",
      ),
  });
}
...
export function useUpdateTechnician(id: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: api.TechnicianFormInput) =>
      api.updateTechnician(id, input),
    onSuccess: () => {
      invalidateTechnicians(queryClient);
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error ? err.message : "Could not update the technician.",
      ),
  });
}
...
export function useSetTechnicianStatus(id: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (status: "active" | "on_leave" | "inactive") =>
      api.setTechnicianStatus(id, status),
    onSuccess: (technician) => {
      invalidateTechnicians(queryClient);
      toast.success(
        `${technician.name} marked as ${technician.status.replace("_", " ")}.`,
      );
    },
    onError: (err) =>
      toast.error(
