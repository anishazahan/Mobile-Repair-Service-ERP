...
export function useAssignTechnician(id: string) {
  return useOrderMutation(
    ({ technicianId, reason }: { technicianId: string; reason?: string }) =>
      api.assignTechnician(id, technicianId, reason),
    "Technician assigned.",
  );
}
...
export function useSubmitDiagnosis(id: string) {
  return useOrderMutation(
    (input: api.SubmitDiagnosisInput) => api.submitDiagnosis(id, input),
...
export function useRecordApproval(id: string) {
  return useOrderMutation(
    (input: api.RecordApprovalInput) => api.recordApproval(id, input),
...
export function useAddProgressNote(id: string) {
  return useOrderMutation(
    (note: string) => api.addProgressNote(id, note),
    "Progress note added.",
  );
}

export function useAddPartsUsed(id: string) {
  // Keeps Spare Parts stock in sync after parts are logged on an order.
  return useOrderMutation(
    (input: api.AddPartsInput) => api.addPartsUsed(id, input),
...
export function usePauseForParts(id: string) {
  return useOrderMutation(
    ({ missingPartName, note }: { missingPartName: string; note?: string }) =>
      api.pauseForParts(id, missingPartName, note),
...
export function useSubmitQualityCheck(id: string) {
  return useOrderMutation(
    (input: api.QualityCheckInput) => api.submitQualityCheck(id, input),
...
export function useConfirmDelivery(id: string) {
  return useOrderMutation(
    (input: api.DeliveryInput) => api.confirmDelivery(id, input),
    "Delivery confirmed.",
  );
}

export function useCloseOrder(id: string) {
  // Keeps Billing in sync since closing generates an invoice.
...
export function useCancelOrder(id: string) {
  return useOrderMutation(
    ({ reason, note }: { reason: string; note?: string }) =>
      api.cancelOrder(id, reason, note),
    "Order cancelled.",
  );
}
